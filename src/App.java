...
////////////////////////////////////////////////////////////////////////
    ...
    private static final    String  PREF_DIMENSIONS_STR     = "The preferred dimensions of the applet are ";
...
////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        private DoAppInitialisation( String[] arguments )
        {
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        public void run( )
        {
            ...
            {
                if ( AppConfig.getInstance( ).isNewDocumentOnStartup( ) )
    ...
    // APPLET INITIALISATION CLASS


    /**
     * The run() method of this class is used to perform the remaining initialisation of the applet from the
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        private DoAppletInitialisation( FuncPlotter applet )
        {
            this.applet = applet;
        }
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        public void run( )
        {
            // Set view as applet's content pane
            FunctionView view = getView( );
            applet.setContentPane( view );

            // Parse startup parameters
            String attrKey = XmlUtilities.appendAttributeName( STARTUP_PARAM_KEY,
                                                               FunctionDocument.X_INTERVAL_KEY );
            String attrValue = applet.getParameter( attrKey );
            if ( attrValue != null )
                getDocument( ).parseStartupParam( FunctionDocument.X_INTERVAL_KEY, attrValue, 0 );

            attrKey = XmlUtilities.appendAttributeName( STARTUP_PARAM_KEY,
                                                        FunctionDocument.Y_INTERVAL_KEY );
            attrValue = applet.getParameter( attrKey );
            if ( attrValue != null )
                getDocument( ).parseStartupParam( FunctionDocument.Y_INTERVAL_KEY, attrValue, 0 );

            for ( int i = 0; i < FunctionDocument.MAX_NUM_FUNCTIONS; ++i )
            {
                attrKey = XmlUtilities.appendAttributeName( STARTUP_PARAM_KEY,
                                                            FunctionDocument.FUNCTION_KEY + i );
                attrValue = applet.getParameter( attrKey );
                if ( attrValue != null )
                    getDocument( ).parseStartupParam( FunctionDocument.FUNCTION_KEY, attrValue, i );
            }
            if ( getDocument( ).hasFunctions( ) )
            {
            ...
            // Test size of applet
            Dimension prefSize = applet.getPreferredSize( );
            String prefDimensionsStr = new String( PREF_DIMENSIONS_STR +
                                                            prefSize.width + " " + WIDTH_STR + " \u00D7 " +
                                                            prefSize.height + " " + HEIGHT_STR + "." );
            if ( (applet.getWidth( ) < prefSize.width) || (applet.getHeight( ) < prefSize.height) )
            {
                System.err.println( DIMENSIONS_ERROR_STR );
                showErrorMessage( SHORT_NAME, DIMENSIONS_ERROR_STR + "\n" + prefDimensionsStr );
            }

            // Print preferred dimensions
            System.out.println( prefDimensionsStr );
        }
...
////////////////////////////////////////////////////////////////////////
    ...
    {
        return ( isApplet ? getDocument( 0 )
                          : hasDocuments( ) ? getDocument( mainWindow.getTabIndex( ) ) : null );
    }
    ...
    {
        return ( isApplet ? getView( 0 )
                          : hasDocuments( ) ? getView( mainWindow.getTabIndex( ) ) : null );
    }
                      ...
                      FuncPlotter applet )
    {
        ...
        debug = isApplet ? (applet.getParameter( DEBUG_PROPERTY_KEY ) != null)
                         : (System.getProperty( DEBUG_PROPERTY_KEY ) != null);

        // Read build properties
        buildProperties = new ResourceProperties( BUILD_PROPERTIES_PATHNAME, getClass( ) );

        // Print applet name and version
        if ( isApplet )
            System.out.println( APPLET_STR + SHORT_NAME + " " + getVersionString( ) );
        ...
        {
            // Create document
            documentsViews.add( new DocumentView( new FunctionDocument( ) ) );

            // Perform remaining initialisation from event-dispatching thread
            SwingUtilities.invokeLater( new DoAppletInitialisation( applet ) );
        }
        ...
        else
        {
            ...
            // Perform remaining initialisation from event-dispatching thread
            SwingUtilities.invokeLater( new DoAppInitialisation( arguments ) );
        }
    }
        ...
        throws AppException
    {
        ...
        try
        {
            ...
            if ( errorStrs.isEmpty( ) )
                ...
                ErrorListDialog.showDialog( mainWindow, ERRORS_STR, Util.getPathname( fileInfo.file ),
                                            errorStrs );
        }
    ...
    private FunctionDocument.FileInfo chooseSave( FunctionDocument.FileInfo fileInfo )
    {
        ...
        if ( saveFileChooser.showSaveDialog( mainWindow ) == JFileChooser.APPROVE_OPTION )
        {
            ...
            if ( fileKind != null )
                file = Util.appendSuffix( file, fileKind.getFilter( ).getSuffix( 0 ) );
    ...
    private File chooseExport( )
    {
        ...
        {
            exportFile = Util.appendSuffix( exportFileChooser.getSelectedFile( ),
                                            AppConstants.PNG_FILE_SUFFIX );
            return exportFile;
        }
        return null;
    ...
    private void openFiles( File[] files )
    {
        for ( int i = 0; i < files.length; ++i )
        {
            ...
            try
            {
                openDocument( new FunctionDocument.FileInfo( files[i], null ) );
                if ( Task.isCancelled( ) )
                    break;
            }
            catch ( AppException e )
            ...
            for ( int i = 0; i < numDocuments; ++i )
            {
                if ( i != currentIndex )
                    strs[index++] = getDocument( i ).
                                        getTitleString( AppConfig.getInstance( ).isShowFullPathnames( ) );
            }
